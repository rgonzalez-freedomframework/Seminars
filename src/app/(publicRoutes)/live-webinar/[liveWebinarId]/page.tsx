...
const page = async ({ params, searchParams }: Props) => {
  ...
  if (!webinarData) {
    return (
    <div className="w-full min-h-screen flex justify-center items-center text-lg sm:text-4xl bg-[#F6F7F4] text-[#1D2A38]">
    Webinar not found
    </div>
  );
  }
    ...
    if (!checkUser.user) {
      return (
        <div className="w-full min-h-screen flex flex-col justify-center items-center gap-4 text-center px-4 bg-[#F6F7F4] text-[#1D2A38]">
    ...
    return (
  	<div className="w-full min-h-screen mx-auto bg-[#F6F7F4] text-[#1D2A38]">
      <RenderWebinar
      error={error}
